...
- **MISS**: a short, gentle sweep targeting approximately 145 ms and roughly
  330–470 Hz, with a soft attack and release so it reads as a water drop.
- **HIT**: a short, approximately 130 ms crack layered with a low report that
  starts near 127 Hz and settles near 92 Hz.
- **DESTROYED**: an approximately 780 ms layered explosion with a strong
  initial impact followed by a decaying low rumble and deterministic noise.

These are the initial listening and verification targets, not external assets.
PLAN may tune individual synthesis constants while preserving the three
audible identities, the relative durations, the conservative level, and
...
4. **Given** the visible target board hides unshot ship cells, **when** a newly
   ...
   alive-ship count decreased in the same state transition, in which case it is
...
- Disabled sound, an unavailable audio capability, a suspended context, a
  rejected resume, output-device suppression, or a document visibility change
  results in best-effort no-op behavior. None may block rendering, toasts,
  tab switching, navigation, or results display.
...
- **FR-001**: The system MUST provide three distinct synthesized feedback
  ...
  explosion-like DESTROYED effect. The destroyed effect MUST be perceptibly
  longer than either short effect, and all effects MUST remain subtle secondary
  feedback to the visual board and toast.
...
- **FR-006**: For the visible target board, a newly shot cell without a visible
  ...
  decreased in that transition, in which case it MUST be classified as
  DESTROYED. The system MUST NOT infer destruction solely from a partially
  revealed target-board ship.
...
- **FR-008**: When multiple real shot cells are newly observed in one state
  ...
  continuing to suppress automatic moat reveals.
...
- **FR-012**: The system MUST treat browser audio as optional. Missing audio
  capability, suspended or rejected audio activation, unavailable output, and
  cleanup during unmount MUST not throw into gameplay rendering, produce an
  unhandled rejection, or prevent existing visual feedback and navigation.
...
- English and Ukrainian resources MUST contain matching keys for the Sound
  control's on/off states and all new accessible, loading, or failure text.
...
- **Gameplay snapshot baseline**: The previous full gameplay state used to
  ...
  establishes the baseline without producing feedback.
...
- **SC-001**: Focused classification and gameplay tests report exactly one
  ...
  auto-revealed moat cells.
- **SC-002**: Audio-adapter tests cover all three recipes and confirm that each
  schedules its expected synthesized components, approximate relative duration
  (short MISS/HIT and longer DESTROYED), conservative gain, and cleanup without
  reading an external audio file.
...
- **SC-004**: English and Ukrainian translation parity tests report identical
  key sets and both locales provide the Sound on/off, unlock/failure, and
  accessible status text required by the gameplay control.
- **SC-005**: Failure-path tests cover missing browser audio capability,
  suspended or rejected activation, unavailable output, rapid updates, and
  unmount cleanup with zero unhandled promise rejections and no interruption
  to visual rendering, toast display, tab switching, or results navigation.
...
- Exact oscillator, noise, envelope, filter, and scheduling parameters remain
  tunable implementation constants during PLAN, provided the three observable
  sound identities, relative durations, conservative level, and deterministic
  in-memory generation remain intact.
