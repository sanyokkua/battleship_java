...
- [ ] T003 [P] Create `frontend/src/config/appBasePath.ts` and `frontend/src/config/appBasePath.test.ts` with one pure normalization function and one canonical absolute join-URL builder; accept empty or `/` as `/`, add exactly one leading slash, remove trailing slashes, preserve internal segments and path-only values such as `/battleship/v1`, reject query/hash values and current-route suffixes using the existing route set (`/join`, `/game/wait`, `/game/preparation`, `/game/gameplay`, `/game/results`) as explicit invalid fixtures, and use the URL API with `searchParams.set('id', sessionId)` for encoded IDs; prove hostname, IP, protocol, non-default port, root, empty, duplicated-leading/trailing slash, internal segment, path-only, invalid query/hash/current-route, and encoded-ID cases with `frontend/node/npm --prefix frontend run test -- src/config/appBasePath.test.ts` (FR-005, FR-006, SC-002, CHK004-CHK005, CHK009-CHK011, CHK020, CHK029, CHK031, CHK037-CHK038).

- [ ] T004 Wire the normalized value from `frontend/src/config/appBasePath.ts` into `frontend/vite.config.ts` as the Vite asset `base` and into `frontend/src/index.tsx` as `BrowserRouter basename`, using `loadEnv` or an equivalent module-safe input path without duplicating normalization rules; add configuration assertions for root and `/battleship/`, run `frontend/node/npm --prefix frontend run build` and `VITE_APP_BASE_PATH=/battleship/ frontend/node/npm --prefix frontend run build`, inspect `frontend/build/index.html` and emitted asset references, and verify an invalid query/hash/current-route setting fails deterministically before a share URL is accepted (FR-005, FR-006, SC-002, CHK010, CHK014-CHK016, CHK031, CHK038). Prove with the two build commands, the focused configuration test, and the expected non-zero invalid-configuration command result.
...
- [ ] T005 [US1] Implement `frontend/src/widgets/sharing/QrCodeSheet.tsx` and `frontend/src/widgets/sharing/QrCodeSheet.css` with the existing `Sheet` as the sole overlay boundary, dynamic `qrcode` import only after `open`, `toCanvas` options `width: 300`, `margin: 2`, error correction `M`, idle/loading/ready/error state, hidden canvas until fulfillment, cleanup protection for late results, localized dialog/status/alert/Close content, ready canvas `role="img"` with localized `aria-label`, and no copy/download/export/customization/navigation controls; add the complete English/Ukrainian keys in `frontend/src/i18n/en/screens.json` and `frontend/src/i18n/uk/screens.json` (`screens.wait.showQr`, `screens.qr.title`, `screens.qr.scan`, `screens.qr.loading`, `screens.qr.description`, `screens.qr.error`, `screens.qr.close`) and co-located contract tests in `frontend/src/widgets/sharing/QrCodeSheet.test.tsx` covering closed/lazy behavior, deferred module/render success, rejection, close/unmount/reopen, and both locale resources; prove with `frontend/node/npm --prefix frontend run test -- src/widgets/sharing/QrCodeSheet.test.tsx src/i18n/keyParity.test.ts` (FR-002-FR-004, FR-007-FR-010, SC-001, SC-004, SC-005, CHK002-CHK007, CHK012-CHK013, CHK016, CHK026-CHK028, CHK032-CHK036).

- [ ] T006 [US1] Update `frontend/src/screens/WaitScreen.tsx`, `frontend/src/screens/WaitScreen.css`, and `frontend/src/screens/WaitScreen.test.tsx` to own `qrOpen`, derive one canonical URL from `frontend/src/config/appBasePath.ts`, pass that exact URL to both Copy link and `QrCodeSheet`, render `Show QR code` beside unchanged Copy ID/Copy link controls only after valid `WAITING_FOR_PLAYERS` context and resolved loading, open the presentation before generation completes, and preserve refresh, polling, navigation, and existing copy behavior; add resolved-valid, missing-session/player, other-status, unresolved-loading, initial-no-QR, open-action, and existing-regression fixtures; prove with `frontend/node/npm --prefix frontend run test -- src/screens/WaitScreen.test.tsx` (FR-001, FR-002, FR-005-FR-006, FR-011, SC-001, SC-006, CHK001-CHK002, CHK008, CHK017, CHK019, CHK024, CHK030, CHK037).
...
- [ ] T008 [US2] Add `frontend/e2e/qr-session-sharing.spec.ts` for the mock-adapter journey: create a session through the real UI, reach `/game/wait`, assert no QR presentation before selection, select `Show QR code`, observe one ready canvas/instruction, and open the canonical join destination with the original encoded ID so `JoinGameScreen` pre-fills it; run the same journey at the configured `/battleship/` prefix by launching the mock server with `VITE_APP_BASE_PATH=/battleship/` and navigating below that prefix, assert asset URLs and router paths contain the prefix exactly once, and use `frontend/e2e/support/mockBackdoor.ts` only for setup/navigation support; prove with `frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` and `VITE_APP_BASE_PATH=/battleship/ frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` (FR-001-FR-006, FR-011, SC-001-SC-003, SC-006, CHK001-CHK005, CHK019-CHK021, CHK024-CHK025, CHK037-CHK038).
