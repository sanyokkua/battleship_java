...
- [ ] T008 [P] [US2] Add `frontend/e2e/qr-session-sharing.spec.ts` for the mock-adapter journey using the valid existing UUID-format session ID `123e4567-e89b-12d3-a456-426614174000`: create the session through the real UI, reach `/game/wait`, assert no QR presentation before selection, select `Show QR code`, observe one ready canvas/instruction with its localized accessible label, and open the canonical join destination so `JoinGameScreen` pre-fills the UUID without changing validation. Run the same journey at the configured `/battleship/` prefix by launching the mock server with `VITE_APP_BASE_PATH=/battleship/` and navigating below that prefix, assert asset URLs use the derived trailing-slash asset base while router paths and join URLs use the logical slashless base exactly once, and use `frontend/e2e/support/mockBackdoor.ts` only for setup/navigation support; prove with `frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` and `VITE_APP_BASE_PATH=/battleship/ frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` (FR-001-FR-006, FR-011, SC-001-SC-003, SC-006, CHK001-CHK005, CHK019-CHK021, CHK024-CHK025, CHK037-CHK038).
...
- T007 and T008 depend on the integrated screen/helper behavior in T006. T009
  ...
  and can run in parallel with T007-T009 because it owns only
