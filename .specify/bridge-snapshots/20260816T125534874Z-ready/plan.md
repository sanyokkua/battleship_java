...
the single environment-agnostic configuration/helper boundary and exports
`normalizeAppBasePath(raw: string | undefined): string` plus
`buildCanonicalJoinUrl(origin: string, normalizedBasePath: string, sessionId: string): string`.
...
route suffixes and empty/whitespace session IDs fail closed. The router route
table, adapters, hooks, backend, and OpenAPI remain unchanged.
...
- Configuration/helper: `appBasePath.ts` exports the pure normalizer and canonical URL builder. The normalizer returns the logical path (`/` or a non-root path without a trailing slash) and raises a deterministic configuration error for query/hash/current-route suffixes. `vite.config.ts` loads the raw value with `loadEnv` and derives the slash-terminated Vite asset base only for non-root values; `index.tsx` passes the logical value to `BrowserRouter basename`; `WaitScreen` passes the same logical value to the builder. The builder rejects empty/whitespace session IDs, uses `new URL`, selects `/join` for root or `${basePath}/join` for subpaths, and sets `id` through `searchParams.set`.
...
and configured-subpath mock-browser journeys.
...
- T007, T008, and T009 run in parallel after T006. T010 runs in parallel with
  T007-T009 after T005 because it owns CSS/build files only. T011 depends on
...
| Waiting-room and join behavior | `frontend/src/screens/WaitScreen.test.tsx` and `frontend/src/screens/JoinGameScreen.test.tsx` | FR-001, FR-005-FR-006, FR-011, SC-001-SC-003, SC-006 |
...
| Visible user journey | `frontend/e2e/qr-session-sharing.spec.ts` under the mock adapter, including a 320px viewport, dismissal/reopen, localization, and join navigation; deterministic module/render failure remains focused-test evidence | FR-001-FR-003, FR-005, FR-007, FR-010-FR-011, SC-001, SC-003-SC-006 |
