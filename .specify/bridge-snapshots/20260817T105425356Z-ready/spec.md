...
- At the canonical-URL helper boundary, any non-empty session identifier is
  ...
  `/`, `%`, and non-ASCII characters; these escaped inputs are
  helper/consumer-only fixtures and are not valid browser-journey join inputs.
  The create/share browser journey uses the opaque ID issued by the existing
  mock adapter to prove canonical transport and exact join-field prefill
  without submission.
  A separate valid-UUID fixture proves that existing join validation remains
  unchanged.
...
- Base-path boundaries are explicit: an empty setting or `/` normalizes to `/`;
  ...
  `/join` and contains no query, hash, or reserved application-route suffix. A
  reserved suffix is a terminal path sequence equal to `/join`, `/game/wait`,
  `/game/preparation`, `/game/gameplay`, or `/game/results` after trailing-slash
  removal. Matching applies only at the end of the configured path:
  `/battleship/game/wait` is invalid, while `/battleship/game/wait-assets` and
  `/community/join-us` remain valid path-only settings. A configured value with
  a query, hash, or reserved terminal suffix is invalid: shared normalization
  raises the typed configuration error, the build exits non-zero, and no share
  URL is emitted rather than silently stripping the invalid portion.
...
- **FR-003**: In the ready state, the QR presentation MUST contain one
  accessible QR image and a short localized scan instruction. It MUST NOT add
  download, export, duplicate-copy, customization, or navigation controls.
...
- **FR-008**: The QR presentation MUST expose distinct localized loading,
  ready, and generation-error states. Loading and generation-error states MUST
  keep the QR image hidden, and a pending or rejected generation operation MUST
  never be treated as a successful result.
...
- The single build-time application base-path setting is the source of truth
  ...
  excludes query strings, fragments, and the reserved terminal application
  routes defined in the edge-case contract. The current root-served deployment
  therefore uses `/` before `/join`, while a subpath build derives a trailing
  slash only for asset delivery.
