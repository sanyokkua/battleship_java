...
- Base-path boundaries are explicit: an empty setting or `/` normalizes to `/`;
  ...
  value containing a query, hash, or current-route suffix is invalid: the
  helper fails closed and emits no share URL rather than silently stripping it.
- The logical normalized base path is the shared source for routing and join
  URLs. Vite derives its asset base as `/` for root or the logical path with
...
- **FR-003**: The QR presentation MUST contain one accessible QR image, a short
...
- Existing session creation, joining, waiting-state transitions, copy actions,
  navigation, and authorization behavior remain unchanged.
...
- One configured public deployment path is shared by asset delivery, routing,
  ...
  exactly once in routes and join URLs, while Vite alone uses its derived
...
- The ready QR remains readable and square, and at viewport widths down to 320
  CSS pixels it scales proportionally without horizontal overflow. Exact QR
  generator options are implementation-contract decisions rather than
...
- Acceptance evidence covers the waiting-room action, lazy generation, URL
  ...
  parity, and unchanged existing sharing behavior.
...
- Invalid base-path configuration fails closed rather than producing a
  potentially misleading share URL.
- The single build-time application base-path setting is the source of truth
  ...
  build derives a trailing slash only for Vite asset delivery.
...
- `JoinGameScreen` remains responsible for consuming the `id` query parameter;
  this feature does not change join validation or navigation. End-to-end
  journeys therefore use existing UUID-format session IDs; escaped-character
  cases are defensive canonical-URL helper fixtures only.
