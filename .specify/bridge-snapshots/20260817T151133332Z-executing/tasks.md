...
- [ ] T022 Preserve every genuine same-update shot event while suppressing only automatic moat reveals per FR-007 and FR-008 (partial). Update `frontend/src/logic/gameplayFeedback.ts` and its focused tests so a transition containing a newly destroyed ship, automatically revealed moat cells, and any independently real newly shot water cells emits the destroyed event plus each real MISS in deterministic player-then-opponent row-major order; keep visibility-only target changes, duplicate snapshots, and automatic moat cells silent. Remain within the approved frontend-only/API-unchanged boundary, or record the explicit contract limitation if the current DTO cannot distinguish those cases.
