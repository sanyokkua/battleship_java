...
same normalizer, with no duplicated normalization rules. Query/hash values and
reserved terminal route suffixes throw `AppBasePathConfigError` with code
`INVALID_APP_BASE_PATH`. Reserved matching is end-anchored after trailing-slash
removal for `/join`, `/game/wait`, `/game/preparation`, `/game/gameplay`, and
`/game/results`; `/battleship/game/wait` is invalid while
`/battleship/game/wait-assets` remains valid. An invalid configured build exits
non-zero. Empty or
...
T009 covers Sheet-backed accessibility, deterministic pending/failure behavior,
and lifecycle recovery in both locales; T010 covers responsive styling/build
proof; and T011 covers stable visible browser dismissal, ready-state reopen,
localization, and narrow-viewport behavior. Deterministic pending, library, and
renderer-failure states remain focused-test evidence.
...
| T011 | `feature/qr-session-sharing--mock-browser-accessibility` | Add stable visible dismissal, ready-state reopen, localization, and narrow-viewport browser proof. |
...
| Visible user journey | `frontend/e2e/qr-session-sharing.spec.ts` under the mock adapter, including exact opaque adapter-ID transport/prefill without submission, a separate valid-UUID direct-route validation fixture, a 320px viewport, dismissal/reopen, localization, and join navigation; deterministic payload equality, pending state, and module/render failure remain focused-test evidence | FR-001-FR-003, FR-007, FR-010-FR-011, SC-001, SC-003-SC-005 |
...
The browser journey cannot decode a canvas as a physical phone would. The exact
QR payload and Copy-link equality are proven by focused clipboard and
`toCanvas` boundary assertions. The browser journey constructs the expected
destination from its observed origin, configured logical base path, and
adapter-issued ID, then proves the rendered presentation, route resolution,
and exact prefill without treating that construction as consumer-equality
evidence. The separate valid-UUID fixture proves the existing join validation
boundary without changing mock-adapter behavior.
