...
- 1.1.0 -> 1.2.0 (2026-09-21): Lean extraction of applicable engineering-governance guidance from a
  ...
  repository; explicitly did not adopt the report's multi-maintainer apparatus (GOV-* rule IDs, roles,
  CODEOWNERS, voting thresholds, a generated AGENTS.md sync block, or a governance CI linter).
- Version change: 1.1.0 -> 1.2.0
- Modified principles: I. Contract Before Implementation (added pre-release contract-mutability rule:
  v1 stays v1, not frozen, fix-in-place with current dependents updated, no migration machinery);
  IV. Practical Proof of Behavior (added a preference for real collaborators/black-box tests over mocks,
  and a requirement that delivered functionality be integrated and tested in the same change);
  VI. Independent Artifacts and Current Evidence (added that saved agent memory and AGENTS.md are
  governed artifacts that must be updated or pruned in the same work item that changes the facts they
  record)
- Added principles: IX. Economical Implementation (DRY, KISS, YAGNI, SOLID; reuse before new; no
  speculative code, DTOs, models, or interfaces; abstractions only once a second concrete implementation
  is known)
- Added sections: none
- Removed sections: none
- Follow-up TODOs: none. The prior follow-up TODO (synchronize AGENTS.md with the constitution) is
  closed: AGENTS.md and the affected auto-memory file were updated in this same change, outside this
  skill's scope guard.
-->
...
The contract remains v1 for the duration of this rewrite and is not frozen. A
gap or design flaw discovered while implementing the backend or frontend MAY
be fixed by amending `openapi.yaml` directly, following the same update-first
ordering above, provided every current dependent is updated in the same
change. The project MUST NOT build migration, deprecation, or versioning
machinery for this pre-release contract.
...
high-entropy capability handling with server-side digest storage for the browser
session (an unused one-time invitation secret MAY stay in process memory so the
host's own snapshot can show the invitation link); a public game
...
Tests MUST prefer real collaborators and observable behavior over mocks or
stubs; a test double is justified only when the real collaborator is
external, non-deterministic, or not yet built in this change. Delivered
functionality MUST be integrated into its consuming code and exercised by a
test in the same change — code with no caller and no test is not accepted as
complete.
...
Saved agent memory and `AGENTS.md` are governed artifacts too: a change that
alters the facts they record MUST update or remove the affected memory and
`AGENTS.md` content in the same work item. Memory MUST NOT retain information
that is expected to change soon or has already gone stale.
...
### IX. Economical Implementation

Code MUST be added only when this change has a concrete caller or test for
it; a function, DTO, model, or interface built for an anticipated future need
MUST wait until the need is real (YAGNI). Before writing new code, an
existing function, component, or type with the needed or closely similar
behavior MUST be reused instead of duplicated (DRY); a new implementation is
justified only when no suitable one exists. Design MUST favor the simplest
structure that satisfies the current requirement (KISS) and single-purpose,
substitutable components (SOLID) over speculative generality. An interface or
abstract class MUST NOT be introduced until a second concrete implementation
is known to exist; anticipated variation alone does not justify the
abstraction.

**EARS rule — Speculative code:** When a change proposes a function, type, or
abstraction with no current caller, test, or second implementation, the
proposal MUST be deferred until that consumer exists.
