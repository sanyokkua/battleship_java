...
**Implementation Evidence**: All T001–T023 tasks are complete. See
`verification.md` for focused, browser, packaged-live, and full-gate evidence.
...
- Q: What should happen when one full snapshot contains both a newly destroyed ship and newly shot water cells next to it? → A: Keep the feature frontend-only and do not add event-origin data to the API. Because the snapshot cannot distinguish a real water shot from an automatic moat reveal, treat adjacent newly shot water cells as automatic moat and suppress their feedback. The feature does not guarantee a separate event for an ambiguous same-update water shot.
...
6. **Given** several genuinely new shot cells arrive in one state update,
   **when** the update is classified, **then** each distinguishable real shot
   produces its own outcome event and auto-revealed moat cells remain
   suppressed. If newly shot water cells are adjacent to a newly destroyed
   ship in the same snapshot, those ambiguous cells are treated as automatic
   moat and produce no separate event.
...
- **FR-007**: The system MUST ignore target-board cells that merely become
  ...
  automatically revealed by a newly destroyed ship. When a newly shot water
  cell is adjacent to a newly destroyed ship in the same snapshot, the cell
  MUST be treated as an automatic moat reveal because the unchanged snapshot
  contract cannot identify its origin.
- **FR-008**: When multiple distinguishable real shot cells are newly observed
  in one state update, the system MUST preserve one outcome event for each
  distinguishable real shot while continuing to suppress automatic moat
  reveals. Events MUST be emitted in deterministic order: `playerField`
  row-major first, followed by `opponentField` row-major. A real water shot
  that is indistinguishable from an automatic moat reveal is intentionally not
  guaranteed to produce a separate event.
...
- **SC-001**: Focused classification and gameplay tests report exactly one
  matching MISS, HIT, or DESTROYED event for every representative
  distinguishable newly observed real shot on both boards, with zero events
  for the initial state, equivalent duplicate snapshots, target-board
  visibility-only changes, auto-revealed moat cells, and ambiguous
  destroyed-ship-adjacent water cells.
