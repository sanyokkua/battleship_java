...
- [X] T008 [P] [US2] Add `frontend/e2e/qr-session-sharing.spec.ts` for the existing mock adapter without changing or aliasing its IDs: create a session through the real UI, capture the opaque adapter-issued session ID from the existing persisted/session UI state, reach `/game/wait`, assert no QR presentation before selection, select `Show QR code`, and observe one ready canvas/instruction with its localized accessible label. Derive the expected join destination inside the test from the observed browser origin, the configured logical base path, and that captured ID solely to prove root/subpath route resolution and exact `JoinGameScreen` prefill; do not describe this construction as QR decoding or Copy-link/QR consumer-equality evidence, which T007 owns at the focused clipboard and `qrcode.toCanvas` boundaries. Do not submit the opaque ID or claim it passes UUID validation. In the same suite, navigate directly to the join route with valid UUID `123e4567-e89b-12d3-a456-426614174000` and assert the existing validation/submission boundary remains enabled and unchanged. Run both proofs at root and at configured `/battleship/` by launching the mock server with `VITE_APP_BASE_PATH=/battleship/`, assert asset URLs use the derived trailing-slash asset base while router paths and derived expected join destinations use the logical slashless base exactly once, and use `frontend/e2e/support/mockBackdoor.ts` only for existing setup/state inspection/navigation support; do not change mock-adapter behavior or add test-only production hooks. Prove with `frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` and `VITE_APP_BASE_PATH=/battleship/ frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` (FR-001-FR-003, FR-011, SC-001, SC-003, CHK001-CHK003, CHK019, CHK021, CHK024-CHK025, CHK037-CHK038).
...
- [X] T009 [P] [US3] Extend `frontend/src/widgets/sharing/QrCodeSheet.test.tsx`, `frontend/src/design/components/Sheet/Sheet.test.tsx`, and `frontend/src/i18n/keyParity.test.ts` with deferred, deterministic QR-module/render controls and parameterized English/Ukrainian cases. Prove role `status` loading, Close while import or rendering is pending, ignored late success and rejection after close/unmount, role `alert` generation failure, fresh loading-to-ready recovery on reopen, ready canvas `role="img"` with the localized `screens.qr.description` `aria-label`, visible localized scan instruction, and usable localized Close for loading/error/ready in both locales. Also cover dialog/`aria-modal`/localized title wiring through the unchanged `Sheet`, focus entering/restoring, Escape and backdrop dismissal, no export/copy controls, and exact key parity; do not add an `aria-describedby` requirement, browser timing dependency, browser fault injection, or reimplement/weaken `Sheet`. Prove with `frontend/node/npm --prefix frontend run test -- src/widgets/sharing/QrCodeSheet.test.tsx src/design/components/Sheet/Sheet.test.tsx src/i18n/keyParity.test.ts` (FR-003, FR-007-FR-010, SC-004-SC-005, CHK003, CHK006-CHK007, CHK013, CHK016, CHK018, CHK022, CHK026-CHK028, CHK032-CHK036).

- [X] T010 [P] [US3] Complete responsive styling only in `frontend/src/widgets/sharing/QrCodeSheet.css` so the nominal canvas is 300 CSS pixels with proportional `max-width: 100%` behavior inside the existing overlay, fits at a 320px viewport without horizontal overflow, and retains centered desktop and mobile bottom-sheet behavior; do not modify T006-owned `frontend/src/screens/WaitScreen.css`, keep computed-size/overflow proof in T011, and prove the style/build contract with `frontend/node/npm --prefix frontend run lint` and `frontend/node/npm --prefix frontend run build` (FR-007, SC-004, CHK012-CHK013, CHK028, CHK033-CHK035).

- [X] T011 [US3] Extend `frontend/e2e/qr-session-sharing.spec.ts` to exercise only stable visible behavior through real controls after the ready state is observable: Close button, Escape, backdrop, focus restoration to `Show QR code`, reopen to a fresh ready presentation, narrow 320px bottom-sheet layout without horizontal overflow, and English/Ukrainian action/title/instruction/Close text. T005/T009 exclusively own deterministic pending-close, loading/error content in both locales, late-result, and recovery evidence through deferred focused controls; do not add browser timing assertions or fault injection. Record browser capability limitations separately from product failures and prove with `frontend/node/npm --prefix frontend run test:e2e -- e2e/qr-session-sharing.spec.ts` (FR-007, FR-010, SC-004-SC-005, CHK006-CHK007, CHK013, CHK018, CHK022, CHK026-CHK028, CHK032-CHK036).
